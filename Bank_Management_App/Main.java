...
public class Main {
    ...
    public static void loadTradeAccounts() throws IOException {
        Path accountPath = paths[0];
        List<String> lines = Files.readAllLines(accountPath);
        lines.stream()
                .forEach(line -> {
                    String[] words = line.split(" ");
                    if (words[0].equals(TradeAccountType.CASH.toString())) {
                        CashAccount cashAccount = new CashAccount(words[1], new BigDecimal(words[2]));
                        cashAccountService.createTradeAccount(cashAccount);
                    } else {
                        MarginAccount marginAccount = new MarginAccount(words[1], new BigDecimal(words[2]));
